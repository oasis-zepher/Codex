...
def render_template(target_date: date) -> str:
    return DAILY_TEMPLATE.read_text(encoding="utf-8").replace("{{date}}", target_date.isoformat())


def main() -> None:
    parser = argparse.ArgumentParser(description="Create today's research diary note.")
    ...
    if not daily_note.exists():
        daily_note.write_text(render_template(target_date), encoding="utf-8")
